...
if (do_preprocess):
    ...
    data_features_list = []
    files_index = int(len(files)/BATCH_SIZE)
    print("Because total file size is large. That is why running in batches....\n\n\n")
    for k in tqdm(range(0, BATCH_SIZE)):
        splitted_files = files[files_index*k:files_index*(k+1)] 
        print("Data matrix has been made out of text dataset files....\n\n\n")
        for i in tqdm(range(0, len(splitted_files))):
            data_features_list.append(DatasetCreation(splitted_files[i], include_label = False).DataBeforeKmeans())
        ...
        DatasetCreation.generateCSVFileFromCodeBook(codebook_list, files, "dataset"+str(k)+".csv")
